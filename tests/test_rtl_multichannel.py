...
TB = ROOT / "rtl" / "tb" / "tb_multi_channel_acquisition_pipeline.sv"
...
def test_multichannel_trigger_arbitration_and_metadata(tmp_path):
    iverilog, vvp, env = _simulator_tools()
    output = tmp_path / "multichannel.vvp"
    ...
    assert "PASS RTL-004 channels=4 simultaneous_priority=CH1_then_CH3 seq=1" in run_result.stdout
